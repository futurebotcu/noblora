-- =============================================================================
-- MATURITY SCORING & TIER PROMOTION SYSTEM
-- Relative tiers: Noble=top 10%, Explorer=next 40%, Observer=bottom 50%
-- Created: 2026-04-01
-- =============================================================================

-- ═══════════════════════════════════════════════════════════════════
-- 1. PROFILES: Add maturity scoring columns
-- ═══════════════════════════════════════════════════════════════════

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS maturity_score             DOUBLE PRECISION NOT NULL DEFAULT 0,
  ...
  ADD COLUMN IF NOT EXISTS follow_through_score       INT NOT NULL DEFAULT 50,
  ADD COLUMN IF NOT EXISTS vitality_score             DOUBLE PRECISION NOT NULL DEFAULT 100;

-- ═══════════════════════════════════════════════════════════════════
-- 2. RPC: Update last active timestamp
-- ═══════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════
-- 3. RPC: Calculate maturity score for one user
-- ═══════════════════════════════════════════════════════════════════
...
DECLARE
  v_p DOUBLE PRECISION := 0;   -- Profile completeness
  v_c DOUBLE PRECISION := 0;   -- Community
  v_d DOUBLE PRECISION := 0;   -- Depth
  v_t DOUBLE PRECISION := 0;   -- Trust
  v_f DOUBLE PRECISION := 50;  -- Follow-through
  v_v DOUBLE PRECISION := 0;   -- Vitality
...
BEGIN
  ...
  IF v_profile.is_verified THEN v_p := v_p + 20; END IF;
...
-- ═══════════════════════════════════════════════════════════════════
-- 4. RPC: Recalculate tiers (relative ranking)
-- ═══════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_rec RECORD;
BEGIN
  -- First, recalculate all maturity scores
  ...
  WITH ranked AS (
    SELECT id, maturity_score,
           ROW_NUMBER() OVER (ORDER BY maturity_score DESC) AS rank
  ...
  SELECT nt.id, 'tier_promoted',
    ...
    END,
    jsonb_build_object('new_tier', nt.new_tier)
  FROM new_tiers nt
  JOIN public.profiles p ON p.id = nt.id
  WHERE p.nob_tier IS DISTINCT FROM nt.new_tier
    AND nt.new_tier IN ('noble', 'explorer');
END;
$$;

-- ═══════════════════════════════════════════════════════════════════
-- 5. RPC: Update vitality decay (runs hourly)
-- ═══════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.update_vitality_decay()
RETURNS VOID LANGUAGE sql SECURITY DEFINER AS $$
  UPDATE public.profiles SET
    vitality_score = CASE
      WHEN last_active_at > NOW() - INTERVAL '24 hours' THEN 100
      WHEN last_active_at > NOW() - INTERVAL '48 hours' THEN 80
      WHEN last_active_at > NOW() - INTERVAL '7 days' THEN 50
      WHEN last_active_at > NOW() - INTERVAL '30 days' THEN 20
      ELSE 0
    END
  WHERE vitality_score > 0;
$$;
...
-- Vitality decay: every hour
DO $$ BEGIN PERFORM cron.unschedule('update-vitality-decay'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
SELECT cron.schedule('update-vitality-decay', '0 * * * *', $$
  SELECT public.update_vitality_decay();
$$);

-- Tier recalculation: every 6 hours
DO $$ BEGIN PERFORM cron.unschedule('recalculate-tiers'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
SELECT cron.schedule('recalculate-tiers', '0 */6 * * *', $$
  SELECT public.recalculate_tiers();
$$);
